...
omics_files = [
    '../data/TCGA_PanCancer_Data_cleaned/cleaned_TCGA_PanCancerAtlas_Expression_230109.csv',
    '../data/TCGA_PanCancer_Data_cleaned/cleaned_TCGA_PanCancerAtlas_Methylation_230109.csv',
    '../data/TCGA_PanCancer_Data_cleaned/cleaned_TCGA_PanCancerAtlas_Mutation_230109.csv',
    '../data/TCGA_PanCancer_Data_cleaned/cleaned_TCGA_PanCancerAtlas_CNA_230109.csv']
...
def train_classification(dataloader, model, epoch, cancer, fold, optimizer, omics, criterion):
    ...
    with tqdm(dataloader, unit='batch') as tepoch:
        ...
        for batch, data in enumerate(tepoch):
            ...
            embedding_tensor = model.get_embedding(input_x, os_event.size(0), omics)
        ...
        print('train:, Loss: {:.4f} Acc: {:.4f} Precision: {:.4f} Recall: {:.4f} F1: {:.4f}'.format(total_loss, acc,
                                                                                                    precision,
                                                                                                    recall, f1))
...
def test_classification(dataloader, model, epoch, cancer, fold, optimizer, omics, criterion):
    ...
    with tqdm(dataloader, unit='batch') as tepoch:
        ...
        print('test:, Loss: {:.4f} Acc: {:.4f} Precision: {:.4f} Recall: {:.4f} F1: {:.4f}'.format(total_loss, acc,
                                                                                                   precision,
                                                                                                   recall, f1))
...
func_args = [(i, all_epochs, device_ids[i]) for i in range(folds)]
multiprocessing_train_fold(TCGA_Dataset_pretrain, func_args)
