...
fold = 0
patient_ids = joblib.load('data_handle/13-Pan-cancer-5-fold-cv.pkl')
TCGA_file_path = '/home/wfa/project/clue/data/SurvBoard/TCGA/'
train_patient_ids = patient_ids[f'train_{fold + 1}']
test_patient_ids = patient_ids[f'test_{fold + 1}']
val_patient_ids = patient_ids[f'val_{fold + 1}']

pancancer_train_dataset = Pancancer_TCGA_Sur_Board(TCGA_file_path, test_cancer, omics_type, train_patient_ids)
pancancer_test_dataset = Pancancer_TCGA_Sur_Board(TCGA_file_path, test_cancer, omics_type, test_patient_ids)
pancancer_val_dataset = Pancancer_TCGA_Sur_Board(TCGA_file_path, test_cancer, omics_type, val_patient_ids)
...
train_dataloader = DataLoader(pancancer_train_dataset, batch_size=128)
test_dataloader = DataLoader(pancancer_test_dataset, batch_size=1)
val_dataloader = DataLoader(pancancer_val_dataset, batch_size=1)
...
for epoch in range(epochs):
    ...
    train_survival(train_dataloader, model, epoch, 'pancancer', fold, optimizer, omics)
    ...
    if test_c_index > best_c_index:
        ...
        best_test_c_indices = test_c_indices
        best_val_c_indices, _ = cal_c_index(val_dataloader, model,  omics, 'val')
    print(f'{fold} time used: ', time.time() - start_time)

best_test_c_indices.insert(0, 'pancancer')
best_val_c_indices.insert(0, 'pancancer')
cancer_c_index.append(best_test_c_indices)
cancer_c_index.append(best_val_c_indices)

print(best_test_c_indices, best_val_c_indices)
